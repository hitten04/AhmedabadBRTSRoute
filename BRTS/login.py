...
class Window_1:
    def __init__(self, master):
        ...
        self.Lbl_Title = Label(self.Frame, text = 'Ahmedabad Brts Route System', font = ('arial',40,'bold'), bg = 'lightskyblue', fg = 'Black')
        self.Lbl_Title.grid(row = 0, column = 0, columnspan =3, pady = 40)
        
        self.Lbl_Title = Label(self.Frame, text = 'Login', font = ('arial',25,'bold'), bg = 'lightskyblue', fg = 'Black')
        self.Lbl_Title.grid(row = 1, column = 0, columnspan =2, pady = 35)
    ...
    def Login(self):
        ...
        if (u == str('admin') and p == str(12345678)):
            self.__menu__()
        else:
            tkinter.messagebox.askyesno("Login","Error : Wrong Credentials")
            self.Username.set("")
            self.Password.set("")
